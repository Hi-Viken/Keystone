using SkyFrpPanel.Infrastructure.Extensions;
...
namespace SkyFrpPanel.Infrastructure.Controllers
{
    /// <summary>
    /// web层通用数据处理
    /// </summary>
    ...
    public class BaseController : ControllerBase
    {
        ...
        {
            return FormatDataContent(data);
        }
        protected IActionResult FormatDataContent(object data)
        {

            // 1. 配置 System.Text.Json 序列化选项（替代 JsonSerializerSettings）
            var options = new JsonSerializerOptions
            {
                // 可选：保留 Newtonsoft.Json 对应的常用配置（如美化格式、忽略空值，按需添加）
                WriteIndented = false, // 生产环境建议关闭美化，默认 false，对应 Newtonsoft.Json 的默认格式
                DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull, // 可选：忽略 null 值属性，对标 Newtonsoft.Json 的 NullValueHandling.Ignore
            };

            // 3. 序列化对象（替代 JsonConvert.SerializeObject）
            string jsonData = System.Text.Json.JsonSerializer.Serialize(data, options);

            // 4. 返回 JSON 内容（与原代码一致，无需修改）
