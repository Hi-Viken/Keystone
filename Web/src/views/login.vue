    ...
    if (valid) {
      ...
      userStore
        ...
        .then((loginRes) => {
          ...
          //sessionStorage.setItem('showLoginRecord', 'true')
          //getLoginLogList(2)
          GetLoginRecord().then((logRes) => {
            if (logRes.code === 200) {
              const loginInfo = logRes.data || {}
              // 2.3 构建通知内容（支持HTML换行）

              const currentStatus = getStatusInfo(loginInfo.status)
              const lastStatus = loginInfo.lastLoginTime ? getStatusInfo(loginInfo.lastStatus) : null
        ...
        <div style="padding: 10px 0; line-height: 2;">
          ...
          <div style="margin-top: 8px;">
            ...
            <span style="margin: 0 8px; color: #909399;">|</span>
            <span style="color: ${currentStatus.color}; font-weight: 500;">
          ...
          ${loginInfo.lastLoginTime ? `
            <div style="margin-top: 6px;">
              ...
              <span style="margin: 0 8px; color: #909399;">|</span>
              <span style="color: ${lastStatus.color}; font-weight: 500;">
                ${lastStatus.icon} ${lastStatus.text}
              </span>
              <span style="margin-left: 4px; color: #909399; font-size: 12px;">
