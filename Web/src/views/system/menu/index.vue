<template>
  <div class="app-container">
    ...
    <vxe-table class="menu-table" show-overflow ref="listRef" size="small" :loading="loading"
      ...
      <vxe-column field="menuType" :title="$t('m.menuType')" align="center" width="80">
        <template #default="scope">
          ...
          <el-tag :disable-transitions="true" type="danger" v-else-if="scope.row.menuType == 'L'">{{ $t('m.link')
            }}</el-tag>
        </template>
      </vxe-column>
      <vxe-column field="orderNum" :title="$t('m.sort')" width="110" sortable align="center"
      ...
      <vxe-column field="query" :title="$t('m.route_query')" show-overflow align="center"
        v-if="columns.showColumn('query')"> </vxe-column>
...
const updateTableHeight = () => {
  ...
  requestAnimationFrame(() => {
    document.documentElement.style.setProperty('--menu-table-height', `${document.documentElement.scrollHeight - 245}px`)
  })
}
...
<style scoped>
.menu-table {
  height: var(--menu-table-height, 500px);
}
</style>
